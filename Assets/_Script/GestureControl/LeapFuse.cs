    ...
    void FixedUpdate () {
        ...
        foreach (GameObject controlAttach in controlAttachList)
        {
            foreach (GameObject fusedAttach in fusedAttachList)
            {
                ...
                // Debug.Log("[LeapFuse] " + controlAttach);
                // Debug.Log("[LeapFuse] " + fusedAttach);
                if (Vector3.Distance(controlAttach.transform.position, fusedAttach.transform.position) < 100 && eventSystem.GetComponent<FuseEvent>().IsFuseMappingExist(fusedAttach,controlAttach))
                {
