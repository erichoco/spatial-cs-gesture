...
public class HandController : MonoBehaviour
{
    ...
    {
    	// Debug.Log("[HandController]: pos " + pos);
    	// Debug.Log("[HandController]: count " + count);
        int posRange = 30;
    ...
    protected virtual void FixedUpdate()
    {
        ...
        switch (gestureType)
        {
            ...
            if (frame.Hands.Count > 0)
            {
                ...
                List<int> activeObjects = LeapStatic.GetActiveObjects();
                int index = mapPositionToIndex(moveHandPosition.x + 105, activeObjects.Count);
                int selectedIndex = activeObjects[index];

                eventSystem.SetSelectedGameObject(
                	GameObject.Find(LeapStatic.constructionObject[selectedIndex]),
                	new BaseEventData(eventSystem));
                LeapStatic.CreatePartLeap(selectedIndex);
            }
            break;
